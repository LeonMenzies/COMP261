...
public class RoboGame extends JFrame {
	...
	private void createMenu() {
		 ...
		 * startIntermediate = new JMenuItem("Intermediate Interpreter"); JMenuItem
		 * startAdvanced = new JMenuItem("Advanced Interpreter");
		 * start.add(startSimple); start.add(startIntermediate);
		 * start.add(startAdvanced); startSimple.addActionListener(new ActionListener()
		 * {@Override public void actionPerformed(ActionEvent e) {
		 * load1.setEnabled(false); load2.setEnabled(false); start.setEnabled(false);
		 * worldComp.start(World.INTERPRETER_SIMPLE); } });
		 * 
		 * startIntermediate.addActionListener(new ActionListener() {@Override public
		 * void actionPerformed(ActionEvent e) { load1.setEnabled(false);
		 * load2.setEnabled(false); start.setEnabled(false);
		 * worldComp.start(World.INTERPRETER_INTERMEDIATE); } });
		 * startAdvanced.addActionListener(new ActionListener() {@Override public void
		 * actionPerformed(ActionEvent e) { load1.setEnabled(false);
		 * load2.setEnabled(false); start.setEnabled(false);
